...
def append_text(_outbuffer, text):
    _outbuffer.extend(text.encode("utf-8"))


def decompress_zstd(data):
    import zstandard as zstd
    decompressor = zstd.ZstdDecompressor()
    return next(decompressor.read_from(ZstdDecompressReader(bytes(data)), 100000, 1000000))
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    try:
        ...
        if MAGIC_NO_COMPRESS_START1==_buffer[_offset] or MAGIC_SYNC_ZSTD_START==_buffer[_offset]:
            ...
            client.pubkey_x = bytes(_buffer[_offset+headerLen-crypt_key_len:_offset+headerLen-crypt_key_len//2])
            client.pubkey_y = bytes(_buffer[_offset+headerLen-crypt_key_len//2:_offset+headerLen])
            ...
            if MAGIC_COMPRESS_START2==_buffer[_offset]:
                ...
                tmpbuffer = decompress_zstd(tmpbuffer)
